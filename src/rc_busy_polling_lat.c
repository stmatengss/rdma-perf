...
const size_t bufcisize = 1024 * 1024;
...
ibPostSend(struct ibv_qp *qp, struct ibv_mr *mr, char *txbuf, size_t txbufsize)
{
		...
		static void
ibPostWrite(struct ibv_qp *qp, struct ibv_mr *mr, char *txbuf, size_t txbufsize, uint32_t remote_key, uint64_t remote_addr)
{
		...
		iswr.send_flags = IBV_SEND_SIGNALED;
		iswr.wr.rdma.rkey = remote_key;
		iswr.wr.rdma.remote_addr = remote_addr;
		...
		static void
ibPostWriteUnsig(struct ibv_qp *qp, struct ibv_mr *mr, char *txbuf, size_t txbufsize, uint32_t remote_key, uint64_t remote_addr)
{
...
ibPostSendAndWait(struct ibv_qp *qp, struct ibv_mr *mr, char *txbuf, size_t txbufsize, struct ibv_cq *cq)
{
		...
		static void
ibPostWriteAndWait(struct ibv_qp *qp, struct ibv_mr *mr, char *txbuf, size_t txbufsize, struct ibv_cq *cq, uint32_t remote_key, uint64_t remote_addr)
{

		ibPostWrite(qp, mr, txbuf, txbufsize, remote_key, remote_addr);
		...
		if (iwc.status != IBV_WC_SUCCESS) {
				...
				fprintf(stderr, "send ibv_poll_cq returned failure\n");
				exit(1);
		}
}
		...
		static void
server(struct ibv_qp *qp, struct ibv_mr *mr, struct ibv_cq *rxcq, char *rxbuf, size_t rxbufsize, struct ibv_cq *txcq, char *txbuf, size_t txbufsize, uint32_t remote_key, uint64_t remote_addr)
{
		uint64_t min_rtt = 0xffffffffffffffffUL, max_rtt = 0;

		memcpy(txbuf, &magic_number, sizeof(magic_number));
		...
		for (i = 0; i < NUM_RTTS; i++) {
				...
				void *rxbuf_ci = rxbuf + (sizeof(int) * i) % bufcisize;

				while(*(volatile uint32_t *)rxbuf_ci != magic_number) {
					// printf("NUM: %d\n", i);
				};

				*(volatile uint32_t *)rxbuf_ci = 0;

#ifdef DUAL_SIDE
				if (i % ASYNC_LEN) {
						ibPostWriteUnsig(qp, mr, txbuf, sizeof(int), remote_key, remote_addr + (sizeof(int) * i) % bufcisize);
				} else {
						ibPostWriteAndWait(qp, mr, txbuf, sizeof(int), txcq, remote_key, remote_addr + (sizeof(int) * i) % bufcisize);
				}
#endif
				uint64_t m_a = rdtsc();
				if (m_a - m_b < min_rtt)
		...
		static void
client(struct ibv_qp *qp, struct ibv_mr *mr, struct ibv_cq *rxcq, char *rxbuf, size_t rxbufsize, struct ibv_cq *txcq, char *txbuf, size_t txbufsize, uint32_t remote_key, uint64_t remote_addr)
{
...
main(int argc, char **argv)
{
		...
		struct exchange_params params = {
				ibGetLID(ctxt, ib_port), qp->qp_num, psn, mr->rkey, rxbuf};
		printf("Local  LID 0x%x, QPN 0x%x, PSN 0x%x, RKEY 0x%lx RADDR 0x%lx\n", params.lid, params.qpn, params.psn, params.remote_key, params.remote_addr);
		if (servername)
				params = client_exchange(servername, tcp_port, &params);
		else
				params = server_exchange(tcp_port, &params);
		printf("Remote LID 0x%x, QPN 0x%x, PSN 0x%x, RKEY 0x%lx RADDR 0x%lx\n", params.lid, params.qpn, params.psn, params.remote_key, params.remote_addr);
		...
		if (servername)
				client(qp, mr, rxcq, rxbuf, rxbufsize, txcq, txbuf, txbufsize, params.remote_key, params.remote_addr);
		else
				server(qp, mr, rxcq, rxbuf, rxbufsize, txcq, txbuf, txbufsize, params.remote_key, params.remote_addr);
